...
def main(base_dir, measurement_name, host_username, output_dir, filename, radar_config_file): 
    ...
    ecal_folder = os.path.join(base_dir,measurement_name,host_username)
    
    # notes source 
    print("Loading experiment notes file.")
    with open(os.path.join(base_dir,measurement_name,"doc/description.txt"), 'r') as file:
        data = file.read()  

    print("Creating output file")
    try:
        out_file = h5py.File(os.path.join(output_dir, filename),'x')
    except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d','--base_dir', type=str, help='Full path to the data base directory')
    parser.add_argument('-m','--measurement', type=str, help='Name of the measurement')
    parser.add_argument('-u','--username',type=str,help='The username of the host', default='mmwave')
    parser.add_argument('-f','--filename',type=str,help='The output filename')
    parser.add_argument('-o','--output_dir',type=str,help='The full path to the data output directory')
    parser.add_argument('-c','--config_file',type=str,help='The radar configuration file', default='config.json')
    args = parser.parse_args()
    
    # Specify data path and output names
    #base_dir = "Test_Data"
    #measurement_name = "corner_3m_statiionary"
    #host_username = "mmwave" # it found as a sub dir of ecal meas folder 
    #filename = "mmwave.hdf5"
    ...
    main(base_dir, measurement_name, host_username, output_dir, filename, config_file)
